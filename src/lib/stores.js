...
export const scatterPlotData = derived(
  [evictionData, selectedInvestorType, selectedYear, selectedCensusTracts, dataScales],
  ([$evictionData, $selectedInvestorType, $selectedYear, $selectedCensusTracts, $dataScales]) => {
    if (!$evictionData || !$evictionData.length) return { allPoints: [], bostonAverage: { x: 0, y: 0 } };
    
    // Create data points for scatter plot
    const allPoints = $evictionData.map(tract => {
