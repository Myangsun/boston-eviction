...
export async function loadData() {
  try {
      ...
      if (response.ok) {
        try {
          ...
          if (
            ...
            !bostonNeighborhoodsData.features ||
            !Array.isArray(bostonNeighborhoodsData.features)
          ) {
            console.error(
              "Boston neighborhoods data is missing features array"
            );
            ...
            bostonNeighborhoodsData = {
              ...
              features: [
                createDefaultNeighborhood("Dorchester", [-71.053, 42.3]),
                createDefaultNeighborhood("Back Bay", [-71.08, 42.35]),
              ],
            };
          }
        } catch (error) {
    ...
    try {
      ...
      try {
        const boundariesResponse = await fetch(
          "./data/Metro_Boston_Census_Tracts.geojson"
        );
    ...
    // Load census data
    const census = await csv("./data/census.csv");
    ...
    const allPoints = $evictionData
      .map((tract) => {
        ...
        if ($selectedFlipindex === "median_rent") {
          ...
          if (
            isNaN(x) ||
            tract.median_price_diff === null ||
            tract.median_price_diff === "NA"
          ) {
            return null;
          }
      ...
      allYearsData[yr] = $evictionData
        ...
        .map((tract) => {
          ...
          if ($selectedFlipindex === "median_rent") {
            ...
            x = Math.min(
              +tract.median_price_diff,
              $dataScales.maxMedianPriceDiff
            );
          }
