<script>
  ...
  function updateChart() {
    ...
    let xScale;
    let xMin, xMax; // Add this line to declare the variables
    ...
    // Add this line to actually create the scale
    xScale = d3.scaleLinear()
      .domain([xMin, xMax])
      .range([0, innerWidth]);
      
      
    // Debug data to check actual values
    if (data.allPoints.length > 0) {
      console.log("Sample point values:", data.allPoints.slice(0, 3).map(d => ({
        x: d.x,
        y: d.y,
        x_type: typeof d.x,
        y_type: typeof d.y
    ...
    // Update axes
    // Update axes with appropriate formatting for each indicator
    let xAxis;
    if (Flipindex === 'median_rent') {
      ...
      xAxis = d3.axisBottom(xScale)
        .ticks(5)
        .tickFormat(d => `$${d >= 0 ? '+' : ''}${d.toLocaleString()}`); // Show + for positive values
    }
